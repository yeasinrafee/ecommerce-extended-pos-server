...
import type { CreatePosBillInput, NormalizedPosBillLine, PosProductLineInput } from './pos.types.js';
...
const normalizeCreatePosBillPayload = (payload: CreatePosBillInput) => {
	...
	const storeId = toTrimmedString(payload.storeId);
	if (!storeId) {
		throw new AppError(400, 'Invalid store id', [
			{ field: 'storeId', message: 'storeId is required', code: 'INVALID_STORE_ID' }
		]);
	}
	...
	for (let attempt = 0; attempt < 3; attempt += 1) {
		...
		if (!createdOrder) {
			throw new AppError(500, 'POS order retrieval failed', [
				{ field: 'order', message: 'Could not load created POS order', code: 'POS_ORDER_FETCH_FAILED' }
			]);
		}
