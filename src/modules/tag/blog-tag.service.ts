...
import type { CreateTagDto, UpdateTagDto, ServiceListResult, TagListQuery } from './blog-tag.types.js';

import type { Prisma } from '@prisma/client';

const getTags = async ({ page = 1, limit = 10, searchTerm }: TagListQuery = {}): Promise<ServiceListResult<any>> => {
    const skip = (page - 1) * limit;
    const where: Prisma.BlogTagWhereInput = searchTerm
...
const createTag = async ({ name }: CreateTagDto) => {
    ...
    const existing = await prisma.blogTag.findMany({ select: { id: true, name: true } });
...
const updateTag = async (id: string, payload: UpdateTagDto) => {
    ...
    if (payload.name) {
        ...
        const others = await prisma.blogTag.findMany({ where: { NOT: { id } }, select: { id: true, name: true } });
        ...
        return prisma.blogTag.update({ where: { id }, data: { name: payload.name, slug } });
    }

    return prisma.blogTag.update({ where: { id }, data: payload as any });
};
...
const getAllTags = async () => {
    return prisma.blogTag.findMany({ orderBy: { createdAt: 'desc' } });
};
